...
data_files = {
            'train': "/home/UG/ng0003ck/T5imdb/imdb/imdb_train.json",
            'test': "/home/UG/ng0003ck/T5imdb/imdb/imdb_test.json",
        }
...
def tokenize_func(examples):
    model_inputs = tokenizer(['predict: ' + text for text in examples['review']], max_length=512, truncation=True)
    expl_model_inputs = tokenizer(['explain: ' + text for text in examples['review']], max_length=512, truncation=True)
...
tokenized_dataset = json_dataset.map(tokenize_func, remove_columns=['id', 'review', 'choices', 'answer', 'rationale'], batched=True)
